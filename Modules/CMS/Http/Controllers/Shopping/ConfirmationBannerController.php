...
class ConfirmationBannerController extends Controller
{
                ...
                DB::beginTransaction();
                    ...
                    foreach (Auth::user()->countries as $uC) {
                        ...
                        if ($request->$active != null) {
                            ...
                            $idBan = $this->saveConfirmationCountry($uC->id,$type,$purpose,$request->$link, $request->$active);
                            if ($idBan > 0) {
     ...
     * @return Response
     */
    public function edit($type,$purpose) {

        $confirmationsByCountry = ConfirmationBanner::where('type_id',$type)->where('purpose_id',$purpose)->whereIn('country_id', User::userCountriesPermission())->get();
        ...
        $this->layoutData['content'] = View::make('admin::shopping.confirmationbanners.edit', compact('confirmationsByCountry', 'type', 'purpose','type_list','purpose_list','countriesTo', 'anotherCountries','selectedType','selectedPurpose'));
    }
     ...
     * @return int
     */
    private function saveConfirmationCountry($countryId, $type,$purpose,$link,$active)
    {


        $data = ConfirmationBanner::updateOrCreate(
            ['country_id' => $countryId,'purpose_id' => $purpose,'type_id' => $type],
