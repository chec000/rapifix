...
class GroupsController extends Controller
{
    ...
    public function getPages($groupId)
    {
        ...
        if ($group->exists) {
            ...
            if (!empty($pageIds)) {
                foreach ($pageIds as $pageId) {
                    ...
                    foreach ($attributeBlocks as $attributeBlock) {
                        $pageBlockContent = PageBlock::preloadPageBlockLanguage($pageId, $attributeBlock->id, -1, 'block_id', $pageModel->brand_id, $pageModel->country_id )->content;
            ...
            $this->layoutData['content'] = View::make('admin::cms.pages.groups', array('group' => $group, 'pages' => $pagesTable, 'can_add' => $group->canAddItems(), 'can_edit' => $group->canEditItems()));
        }
    }
    ...
    public function getEdit($groupId)
    {
        ...
        if ($group->exists) {
            ...
            $this->layoutData['content'] = View::make('admin::cms.pages.groups.edit', ['group' => $group, 'defaultTemplate' => $group->default_template, 'templateSelectOptions' => $templateSelectOptions, 'blockList' => $blockList]);
        }
    }
