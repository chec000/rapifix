...
class PagesController extends Controller {
    ...
    public function getPagesByParameters(rqs $request) {
        /* session()->put('filters.brand_id', $request->brand_id);
          session()->put('filters.country_id', $request->country_id);
          session()->put('filters.language_id', $request->language_id); */
...
//            dd("es null");
        ...
        $pageCurrentExist = Page::join('cms_page_lang', 'cms_pages.id', '=', 'cms_page_lang.page_id')->where(
    ...
    public function postEdit($pageId) {
        ...
        if (Request::input('duplicate') == 1) {
            ...
            $targetPage = Page::join('cms_page_lang', 'cms_page_lang.page_id', '=', 'cms_pages.id')
                            ->where('cms_page_lang.url', $currentUrl)->where('cms_page_lang.language_id', $page_info_lang['language_id'])
                            ->where('cms_pages.brand_id', $page_info['brand_id'])
                            ->where('cms_pages.country_id', $page_info['country_id'])->first();
            if ($targetPage != null) {
                $this->addAlert('danger', 'Duplication failed, another page is using the selected brand, country and language.');
