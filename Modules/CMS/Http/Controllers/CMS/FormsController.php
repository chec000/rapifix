    ...
    public function getList($pageId = 0)
    {
        ...
        if (isset($form_blocks)) {
            if (count($form_blocks) == 1) {
                return \redirect()->route('admin.forms.submissions', ['pageId' => $page->id, 'blockId' => $form_blocks[0]->id]);
            }
            ...
            $this->layoutData['content'] = View::make('admin::cms.pages.forms.list', array('page_name' => $name, 'page_id' => $pageId, 'forms' => $form_blocks));
        } else {
    ...
    public function getSubmissions($pageId = 0, $blockId = 0)
    {
        ...
        if (empty($block_data) || $block_data->type != 'form') {
            ...
            foreach ($submissions as $submission) {
                ...
                $submission_rows .= View::make('admin::cms.partials.forms.submissions', array('submission' => $submission_data))->render();
            }
            $this->layoutData['content'] = View::make(
                'admin::cms.pages.forms.submissions',
                array('links' => PaginatorRender::admin($submissions),
                    ...
                    'export_link' => route('admin.forms.csv', ['pageId' => $pageId, 'blockId' => $blockId])
                )
            );
        }
    }
