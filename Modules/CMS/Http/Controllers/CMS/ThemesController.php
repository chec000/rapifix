...
class ThemesController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $this->layoutData['content'] = View::make('admin::cms.pages.themes', ['themes' => $themes, 'themes_count' => Theme::count(), 'blockSettings' => $blockSettings]);
    }
    ...
    public function getList()
    {
        ...
        foreach ($thumbs as $thumb) {
            ...
            if ($thumb->install) {
                ...
                if ($theme_auth['edit']) {
                    $buttons[] = ['href' => route('admin.themes.edit', ['themeId' => $thumb->id]), 'glyphicon' => 'pencil', 'label' => 'Edit'];
                }
                if ($theme_auth['export']) {
                    $buttons[] = ['classes' => ['exportTheme'], 'glyphicon' => 'download', 'label' => 'Export'];
                }
                if ($theme_auth['update']) {
                    $buttons[] = ['href' => route('admin.themes.update', ['themeId' => $thumb->id]), 'glyphicon' => 'flag', 'label' => 'Review Block Changes'];
                }
            }
