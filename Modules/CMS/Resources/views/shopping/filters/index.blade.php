                ...
                <td>
                    <div class="row">
                        <div class="col-xs-3 text-center">
                            <a href="{{ route('admin.filters.categories.show', ['code' => $fil->code, 'idCountry' => 0, 'idCategory' => 0]) }}"
                        ...
                        <div class="col-xs-3 text-center">
                            @if (Auth::action('filters.delete'))
                                <form name="formDel_{{ $fil->code }}" method="POST" action="{{ route('admin.filters.destroy', ['code' => $fil->code]) }}">
                                    ...
                                    {{ method_field('DELETE') }}
                                    <input type="hidden" name="code" value="{{ $fil->code }}"/>
                                    <i class="fa fa-trash itemTooltip" aria-hidden="true" onclick="document.forms['formDel_{{ $fil->code }}'].submit();"
                                       title="{{ trans('admin::shopping.filters.index.delete_category') }}"></i>
                                </form>
                            @endif
                        </div>
                    </div>
