<div class="row">
    ...
    <div class="box" >
        <div class="box-header with-border">
            <h3 class="box-title" >{{trans('admin::shopping.orders.index.list_orders_detail')}} </h3>
        </div>
        ...
        <div class="box-body">
            <section class="invoice">
                    ...
                    <div class="col-xs-12">
                        <h2 class="page-header">
                            ...
                            @if(strtoupper($order['shop_type']) == 'INSCRIPTION')
                                <span>{{trans('admin::shopping.orders.index.inscription')}}</span>
                            @endif
                ...
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">
                        <address>
                            ...
                            <strong>{{trans('admin::shopping.orders.detail.cellphone')}}</strong>: {{ $shipping_address['cellphone'] }}<br>
                            ...
                            <div>
                                ...
                                @endif
                                <b>{{trans('admin::shopping.orders.detail.birth')}}</b> {{ $shipping_address['birthdate'] }} {{--$order[''sponsor_name'] or ''--}}<br>
                                <b>{{trans('admin::shopping.orders.detail.gender')}}</b> {{ $shipping_address['gender'] }} {{--$order[''sponsor_name'] or ''--}}<br>
                                <b>{{trans('admin::shopping.orders.detail.sponsor')}}:</b> {{ $shipping_address['sponsor'] }} {{--$order[''sponsor'] or ''--}}<br>
                                <b>{{trans('admin::shopping.orders.detail.sponsor_name')}}</b> {{ $shipping_address['sponsor_name'] }} {{--$order[''sponsor_name'] or ''--}}<br>
                            </div>
                            @endif
                        </address>
                    </div>
                    ...
                    <div class="col-sm-4 invoice-col">
                        ...
                        <b>{{trans('admin::shopping.orders.index.source')}}:</b> {{ $order->source->source_name }} {{--$order[''corbiz_transaction']--}} <br />
                ...
                <div class="row">
                    <div class="col-xs-12 table-responsive">
                        <table class="table table-responsive">
                            <thead>
                            <tr>
                                <th>{{trans('admin::shopping.orders.detail.qty')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.product')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.sku')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.description')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.points')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.subtotal')}}</th>
                                <th>{{trans('admin::shopping.orders.detail.action')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            @foreach($orderdetail as $od)
                                ...
                                <tr class="row_{{$od->id}} {{$color}}">
                                    <td>{{ $od->quantity }} </td>
                                    <!-- td>{{$od->countryProduct->name}}</td -->
                                    <td>{{!empty($od->countryProduct->name) ? $od->countryProduct->name : $od->products->global_name}}</td>
                                    <td>{{$od->products->sku}}</td>
                                    <td>{{$od->countryProduct->description}}</td>
                                    <td>{{ $od->points }}</td>
                                    <td>${{ number_format($od->final_price,2) }}</td>
                                    @if($can_delete)
                                    <td>
                                        <span class="label label-success" id="item_{{$od->id}}" style="display: none;">{{trans('admin::shopping.orders.detail.added')}}</span>
                                        @if($order->estatus->key_estatus == 'corbiz_error')
                        ...
                        @if($order->estatus->key_estatus == 'corbiz_error' && in_array($order->source->source_name,Config::get('admin.sources')))
                            ...
                            <table class="table table-responsive" id="addnewproducts">
                                ...
                                <tr id="change_products">
                                    ...
                                    <td colspan="7" style="background-color: #ccc;">{{trans('admin::shopping.orders.detail.change_products')}}<i class="fa fa-arrow-down" aria-hidden="true"></i></td>

                                </tr>
                                <tr id="choose_products">

                                    <td>{{trans('admin::shopping.orders.detail.qty')}}:</td>
                                    <td><input type="number" required="" id="cant_prod"></td>
                                    <td>{{trans('admin::shopping.orders.detail.product')}}: </td>
                                    <td>

                                        <select name="prods" id="prod_sel">
                                            <option value="">{{trans('admin::shopping.orders.detail.chooseoption')}}</option>
                                    ...
                                    <button type="button" class="btn btn-success" style="padding: 0 12px" onclick="addListProduct({{$products}})">
                                        <i class="fa fa-plus" aria-hidden="true"></i> {{trans('admin::shopping.orders.detail.add_product')}}
                                    </button>
                                    ...
                                    <button type="button" id="actOrdBtn" class="actOrdBtn btn btn-primary" style="padding: 0 12px"
                                        ...
                                        <i class="fa fa-floppy-o" aria-hidden="true"></i> {{trans('admin::shopping.orders.detail.save_changes')}}
                ...
                <div class="row">
                    ...
                    <div class="col-xs-6">
                        <span class="lead">{{trans('admin::shopping.orders.detail.payment_methods')}}</span><br>
                        ...
                        <p class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                            <strong>{{trans('admin::shopping.orders.detail.corbiz_error')}}: </strong>{{ $order['error_corbiz'] }}
                        </p>
                        <p class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                            <strong>{{trans('admin::shopping.orders.detail.last_modifier')}}:</strong>
                            @if(!empty($order['last_modifier_id']))
                                <span> {{$order->users->name}}</span>
                            @else
                                <span>{{trans('admin::shopping.orders.detail.system')}}</span>
                            @endif
                        </p>
                        <a class="btn btn-info btn-sm" id="showlog">
                            <i class="fa-calendar-o fa"></i> {{trans('admin::shopping.orders.detail.payment_log')}}
                        </a>
                        @if($order->estatus->key_estatus == 'corbiz_error' && in_array($order->source->source_name,Config::get('admin.sources')))
                            <button class="btn btn-info btn-sm" onclick="changeOrderStatus({{$order['id']}})"> <i class="fa-check-square fa"></i> {{trans('admin::shopping.orders.detail.change_status')}}</button>
                    ...
                    <div class="col-xs-6">
                        <div class="table-responsive">
                            <table class="table">
                                <tbody><tr>
                                    <th style="width:50%">{{trans('admin::shopping.orders.detail.subtotal')}}:</th>
                                    <td>${{ number_format($order['total'] - $order['total_taxes'],2)}} </td>
                                </tr>
                                <tr>
                                    <th>{{trans('admin::shopping.orders.detail.tax')}} </th>
                                ...
                                <tr>
                                    <th>{{trans('admin::shopping.orders.detail.total')}}:</th>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                                ...
                                <div class="modal-header">
                                    ...
                                    <h2>{{ trans('admin::shopping.orders.detail.logs') }}</h2>
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <h2 class="page-header">
                                                <i class="fa fa-shopping-cart"></i>
                ...
                <a class="btn btn-info btn-sm pull-right" style="color:#ffffff; text-decoration: none;" href="{{ route('admin.orders.list') }}"><i class="fa-arrow-circle-left fa"></i> {{trans('admin::shopping.orders.detail.go_back')}}</a></button>
