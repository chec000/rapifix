<h1> {{trans('admin::distributorsPool.edit.edit_dist')}}</h1>
<p class="text-right"><a href="{{ route('admin.pool.index') }}">{{trans('admin::brand.form_add.back_list')}}</a></p>

<form id="form-pool" method="POST" action="{{ route('admin.pool.update', [$distributor->distributor_code, $distributor->country_id]) }}">
    {{ csrf_field() }}
            ...
            <div class="col-xs-12">
                ...
                <select disabled name="country" class="form-control">
                    ...
                    @foreach($countries as $country)
                        <option data-corbiz="{{ $country->corbiz_key }}" {{ old('country', $distributor->country_id) == $country->id ? 'selected' : '' }} value="{{ $country->id }}">{{ $country->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <fieldset class="fieldset_gray">
        ...
        <div class="row">
            ...
            <div class="form-group col-xs-12 col-sm-2">
                ...
                <input type="text" name="code" class="form-control" value="{{ old('code', $distributor->distributor_code) }}" required="required">
            </div>

            <!-- Name -->
            <div class="form-group col-xs-12 col-sm-5">
                <label class="control-label">{{ trans('admin::distributorsPool.general.dist_name') }}</label><span class="required">*</span>
                <input type="text" name="name" class="form-control" value="{{ old('name', $distributor->distributor_name) }}" required="required" maxlength="200">
            </div>

            <!-- Email -->
            <div class="form-group col-xs-12 col-sm-5">
                <label class="control-label">{{ trans('admin::distributorsPool.general.dist_email') }}</label><span class="required">*</span>
                <input type="email" name="email" class="form-control" value="{{ old('email', $distributor->distributor_email) }}" required="required" maxlength="200">
            </div>
        </div>
    </fieldset>
    ...
    @if (Auth::action('distributorspool.edit'))
    <div class="row">
        <div class="col-xs-12">
    ...
    <script>
        $(document).ready(function () {
            $('[name=code]').change(function () {
                ...
                $.ajax({
                    url: '{{ route('admin.pool.validate_sponsor') }}',
                    method: 'POST',
