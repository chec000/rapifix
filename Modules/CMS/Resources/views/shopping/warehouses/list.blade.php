                        ...
                        <div class="col-xs-4 text-center">
                            @if (Auth::action('warehouses.delete'))
                                <form name="formDel_{{ $w->id }}" method="POST" action="{{ route('admin.warehouses.destroy', ['code' => $w->warehouse ]) }}">
                                    ...
                                    {{ method_field('DELETE') }}
                                    <input type="hidden" name="code" value="{{ $w->warehouse }}"/>
                                    <i class="fa fa-trash itemTooltip" aria-hidden="true" onclick="document.forms['formDel_{{ $w->id }}'].submit();"
                                       title="{{ trans('admin::shopping.warehouses.index.delete') }}"></i>
                                </form>
                            @endif
                        </div>
                    </div>
    ...
    function disableOrderEstatus(order_estatus_id) {
        $.ajax({
            ...
            success: function (data) {
                let label = $("#status" + order_estatus_id);
                let iconActive = $("#activeOrderEstatus" + order_estatus_id);
                let iconInactive = $("#inactiveOrderEstatus" + order_estatus_id);
                if (data.status === 0) {
