...
<div class="row textbox">
    <div class="col-sm-6">
        ...
        @if (Auth::action('products.upload_file'))
                    ...
                    <a href="{{ route('admin.products.listWarehouses', ['id' => $countryProduct->product->id]) }}"
                       title="{{ trans('admin::shopping.products.index.edit_product') }}" style="text-decoration: none;color: #000;">
                        ...
                        <form id="delete-product-form-{{ $countryProduct->product->id }}" action="{{ route('admin.products.destroy', $countryProduct->product->id) }}", method="POST" style="display: inline">
                            {{ csrf_field() }}
                            <a onclick="document.getElementById('delete-product-form-{{ $countryProduct->product->id}}').submit();" class=" glyphicon glyphicon-trash itemTooltip" href="javascript:{}" title="{{ trans('admin::shopping.products.index.delete_product') }}"></a>
                        </form>
                        @endif
                </td>
            </tr>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $('#upload-file').click(function ()
        {
            ...
            axios.post('{{ route("admin.products.upload_file") }}', data )
