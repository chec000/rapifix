                        ...
                        <div class="col-xs-4 text-center">
                            @if (Auth::action('systems.delete'))
                                <form name="formDel_{{ $sys->code }}" method="POST" action="{{ route('admin.systems.destroy', ['code' => $sys->code]) }}">
                                    ...
                                    {{ method_field('DELETE') }}
                                    <input type="hidden" name="code" value="{{ $sys->code }}"/>
                                    <i class="fa fa-trash itemTooltip" aria-hidden="true" onclick="document.forms['formDel_{{ $sys->code }}'].submit();"
                                       title="{{ trans('admin::shopping.systems.index.delete_category') }}"></i>
                                </form>
                            @endif
                        </div>
                    </div>
