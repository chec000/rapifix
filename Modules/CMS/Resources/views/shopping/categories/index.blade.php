                        ...
                        <div class="col-xs-4 text-center">
                            @if (Auth::action('categories.delete'))
                                <form name="formDel_{{ $cat->code }}" method="POST" action="{{ route('admin.categories.destroy', ['code' => $cat->code]) }}">
                                    ...
                                    {{ method_field('DELETE') }}
                                    <input type="hidden" name="code" value="{{ $cat->code }}"/>
                                    <i class="fa fa-trash itemTooltip" aria-hidden="true" onclick="document.forms['formDel_{{ $cat->code }}'].submit();"
                                       title="{{ trans('admin::shopping.categories.index.delete_category') }}"></i>
                                </form>
                            @endif
                        </div>
                    </div>
